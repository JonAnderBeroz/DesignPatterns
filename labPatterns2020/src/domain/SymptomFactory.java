...
public class SymptomFactory {
	private List<Symptom> createdSymptoms = new ArrayList<Symptom>();
	private List<String> createdSymptomsName = new ArrayList<String>();
	public Symptom createSymptom(String symptomName) {
		...
		List<Double> index1 = Arrays.asList(5.0, 4.8, 3.7, 0.9, 0.8,0.3,0.3);
		...
		if (impact5.contains(symptomName)) {
			impact=5; index= index5.get(impact5.indexOf(symptomName));
				...
				impact3.contains(symptomName)) {impact=3;index= index3.get(impact3.indexOf(symptomName));
		...
		if (impact!=0)  {
			if (digestiveSymptom.contains(symptomName)) {
				if(!createdSymptomsName.contains(symptomName)) {
					Symptom s = new DigestiveSymptom(symptomName,(int)index, impact);
					createdSymptomsName.add(symptomName);
					createdSymptoms.add(s);
					return s;
				}else {
					int symptomIndex = createdSymptomsName.indexOf(symptomName);
					return createdSymptoms.get(symptomIndex);
				}
			}
